...
-- [[1,3,6],[5,12,21],[12,27,45]] in array form
integralTransform :: Matrix -> Array (Int, Int) Int
integralTransform mat = listArray ((0,0),(rowN-1, colN-1)) xs
  where
    (rowN, colN) = (length mat, length (head mat))
    xs = (concat . transpose . map (scanl1 (+)) . transpose . map (scanl1 (+))) mat

-- |
-- 
largestSubMatrix :: Matrix -> Int
largestSubMatrix mat = maximum [subMatrixSum xL xH yL yH | [xL, xH, yL, yH] <- genIndices (length mat)]
  where
