...
def reverse_group(group, k):
    ...
    divIdx = (k * k) % len(body)
    head, tail = body[:divIdx], body[divIdx:]
    if ((k * k) / len(body)) % 2 == 0:
        # head reverted (head flipped odd times) 
        for cnt in range(len(head) / k):
            begin = k * cnt
            end = k * (cnt + 1)
            head[begin:end] = list(reversed(head[begin:end]))
    else:
        # tail reverted (tail flipped odd times)
        for cnt in range(len(tail) / k):
            begin = k * cnt
            end = k * (cnt + 1)
            tail[begin:end] = list(reversed(tail[begin:end]))
